...
public class MemberServiceImpl implements MemberService {
    ...
    @Override
    public Member duplicateCheck(@RequestHeader("jwt-token")  String email) {
        return memberRepository.findByEmail(email)
                .orElseThrow(() -> new IllegalArgumentException("사용할수 없는 아이디입니다."));
    }
    ...
    public void deleteMember(UserDeleteRequestDto requestDto) {
        Member member = memberRepository.findByEmailAndPassword(requestDto.getEmail(),requestDto.getPassword())
                .orElseThrow(()->new IllegalArgumentException("아이디 비밀번호가 일치 하지 않습니다."));
    ...
    public Long createUser(UserCreateRequestDto requestDto) {
         return memberRepository.save(requestDto.toMemberEntity()).getId();
    }

    @Override
    public void login(UserLoginRequestDto requestDto) {

        memberRepository.findByEmailAndPassword(requestDto.getEmail(), requestDto.getPassword())
